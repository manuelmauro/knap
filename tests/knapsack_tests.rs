...
fn test_complex() {
    ...
    assert!(selected_ids.contains(&"B".to_string()), "Item B should be selected.");
    assert!(selected_ids.contains(&"C".to_string()), "Item C should be selected.");
    assert_eq!(selected_ids.len(), 2, "Incorrect number of items selected for complex case.");
}
